...
async function main() {
  ...
  if(process.argv.length < 5) {
    throw new Error("Please specify amount of voting weight to cast to this vote");
  }
  ...
  try {
    ...
    const tx = await ballotContract.vote(indexProposal, votingQuantity);
    console.log("Awaiting confirmations");
    await tx.wait();
    console.log(`${votingQuantity} vote(s) cast to proposal # ${indexProposal}`);
  } catch (err) {
    console.log(err);
  }
}
